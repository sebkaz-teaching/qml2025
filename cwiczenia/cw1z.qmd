...
1. przejdź pod adres https://sv-pb-jupiter.sgh.waw.pl/j1## w miejsce ## wstaw przypisany w zadaniu indywidualny numer środowiska.
...
pobierz plik [`zad1.ipynb`](https://github.com/sebkaz-teaching/cwiczenia/blob/main/QML/zad1.ipynb) i zrealizuj ćwiczenia.
...
- korzystając z metody `torch.linspace()` wygenruj 500  punktów danych  dla zakresu (0,10) w tablicy `x`
- Ze względu, iż potrzebujemy 500 wierszy przypadków (a nie 500 zmiennych) jednowymiarowej tablicy zastosuj metodę `view(-1,1)`
- jako wynik `si` wygeneruj wartości funckji `torch.sin(x)`.
- Analogicznie jak dla danych `x` pamiętaj o zmianie widoku: `view(-1,1)`

Do zmiennej `si` dodamy wartości losowe generując target.

- $si + 0.1*(\text{torch.rand}(500).\text{view}(-1,1)-0.5)$
